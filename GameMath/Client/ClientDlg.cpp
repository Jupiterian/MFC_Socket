...
BOOL CClientDlg::OnInitDialog()
{
	...
	m_list.InsertColumn(0, L"Пользователь", LVCFMT_CENTER, 200);
	m_list.InsertColumn(1, L"Счёт", LVCFMT_CENTER, 70);
...
void CClientDlg::NewLevel()
{
	g->level++;
	CString str;
	str.Format(L"%s%d", L"Уровень ", g->level);
	GetDlgItem(IDC_LEVEL)->SetWindowTextW(str);
	str.Format(L"%s%d", L"Счёт:", g->points);
	GetDlgItem(IDC_COUNT)->SetWindowTextW(str);
	g->sec = 30;
	str.Format(L"%s%d", L"Оставшееся время: ", g->sec);
